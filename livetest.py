...
RECORDING_DURATION = 240  # 4 minutes (in seconds)
...
if __name__ == "__main__":
    ...
    # Convert schedule time to local time
    schedule_time_sydney = datetime.datetime.strptime(SCHEDULE_TIME, '%H:%M')
    schedule_time_sydney = SYDNEY_TZ.localize(schedule_time_sydney)
    local_time = schedule_time_sydney.astimezone(datetime.datetime.now().astimezone().tzinfo)
    local_schedule_time = local_time.strftime('%H:%M')

    # Schedule the job using the correct day and local time
